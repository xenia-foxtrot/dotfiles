		...
		config = function()
			...
			require("mason-lspconfig").setup({
				...
				handlers = {
					...
					["jdtls"] = make_server_handler(function(server)
						-- Bemol is an external dependency that integrates LSP servers with brazil
						local function bemol()
							local bemol_dir = vim.fs.find(
								{ ".bemol" },
								{ upward = true, type = "directory" }
							)[1]
							local ws_folders_lsp = {}
							if bemol_dir then
								local file =
									io.open(bemol_dir .. "/ws_root_folders", "r")
								if file then
